...
interface RunResponse {
  runId?: string;
  /** Legacy /api/programs/tripwire/run shape. */
  phraseId?: string;
...
export function TripwireRunForm(): ReactNode {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      router.push(body.receiptUrl ?? `/programs/tripwire/runs/${body.runId}`);
    } catch (err) {
      system.facts.errorMessage =
  ...
  return (
    <form onSubmit={onSubmit} data-testid="tripwire-run-form">
      ...
      {requiresPolicyUrl ? (
        <>
          <StudioLabel text="Custom policy URL">
            <StudioInput
              type="url"
