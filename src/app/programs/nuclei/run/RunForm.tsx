...
interface RunResponse {
  runId?: string;
  /** Legacy /api/programs/nuclei/run shape. */
  phraseId?: string;
...
export function NucleiRunForm(): ReactNode {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      // /api/programs/nuclei/run route stays alive as a deprecated alias
      ...
      router.push(body.receiptUrl ?? `/programs/nuclei/runs/${body.runId}`);
    } catch (err) {
      system.facts.errorMessage =
  ...
  return (
    <form onSubmit={onSubmit} data-testid="nuclei-run-form">
      {isPrefilledFromSbomAi ? (
        ...
        >
          Pre-filled from{" "}
          <a href="/programs/sbom-ai" style={{ textDecoration: "underline" }}>
            SBOM-AI receipt
          </a>{" "}
          — sbomRekorUuid + packName already locked. Review the rest and submit.
        </div>
      ) : null}
      <StudioLabel text="Author">
        <StudioInput
          type="text"
          name="author"
          required
      ...
      <StudioHelpText>
        ...
        <code>/programs/nuclei/{`{author}`}/{`{pack}`}</code>.
      </StudioHelpText>

      <StudioLabel text="Pack name + version">
        <StudioInput
          type="text"
          name="packName"
          required
      ...
      <StudioLabel text="SBOM-AI Rekor UUID">
        <StudioInput
          type="text"
          name="sbomRekorUuid"
          required
          placeholder="64–80 hex chars (from /programs/sbom-ai/run)"
          value={sbomRekorUuid ?? ""}
      ...
      <StudioHelpText>
        ...
        <a href="/programs/sbom-ai/run" target="_blank" rel="noreferrer">
          /programs/sbom-ai/run
        </a>{" "}
      ...
      <StudioLabel text="Recommended cron interval">
        <StudioInput
          type="text"
      ...
      {!recommendedIntervalIsValid && (recommendedInterval ?? "").length > 0 ? (
        <StudioHelpText error testId="recommended-interval-invalid">
