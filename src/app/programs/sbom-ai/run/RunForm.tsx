...
interface RunResponse {
  runId?: string;
  /** Legacy /api/programs/sbom-ai/run shape. */
  phraseId?: string;
...
export function SbomAiRunForm(): ReactNode {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      router.push(body.receiptUrl ?? `/programs/sbom-ai/runs/${body.runId}`);
    } catch (err) {
      system.facts.errorMessage =
  ...
  return (
    <form onSubmit={onSubmit} data-testid="sbom-ai-run-form">
      ...
      <StudioLabel text="Expected sha256 (optional)">
        <StudioInput
          type="text"
          name="expectedSha256"
