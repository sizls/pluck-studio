...
| `src/lib/mcp/build-manifest.ts` | **Auto-generator — parallel to OpenAPI.** Pure function `buildManifest({ baseUrl, version })` over `ACTIVE_PROGRAMS` + `BUREAU_PIPELINES`. Emits the Model Context Protocol manifest (`pluck://program/<slug>` resources, `pluck.search` / `pluck.diff` / `pluck.run` tools with JSON-Schema input shapes, prompts, auth). Deterministic; snapshot-tested. Adding a Bureau program auto-extends the manifest. The route at `src/app/api/mcp/manifest.json/route.ts` serves it under the same public-read posture as `/openapi.json` (5-min cache, same-site CSRF + rate-limit, no auth). |
