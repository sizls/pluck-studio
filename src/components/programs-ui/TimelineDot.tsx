...
// onto a horizontal Pluck timeline. Three tones:
...
export interface TimelineDotMarkProps {
  dot: TimelineDot;
  /** Optional href override – defaults to `/programs/<program>/<vendor>/<model>/<dotId>`. */
  href?: string;
}
...
export function TimelineDotMark({ dot, href }: TimelineDotMarkProps): ReactNode {
  const url =
    href ??
    `/programs/${dot.program}/${encodeURIComponent(dot.subject.vendor)}/${encodeURIComponent(dot.subject.model)}/${dot.dotId}`;
  const tooltip =
