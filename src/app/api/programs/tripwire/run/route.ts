// ---------------------------------------------------------------------------
// POST /api/programs/tripwire/run — TRIPWIRE configure endpoint (DEPRECATED ALIAS)
// ---------------------------------------------------------------------------
//
...
export async function POST(req: Request): Promise<Response> {
    ...
    return NextResponse.json(
      {
        ...
        signInUrl: "/sign-in?redirect=/programs/tripwire/run",
      },
      { status: 401 },
    );
  }
  ...
  const body = parsed.value as TripwireRequestBody;
