...
//
// Renders via shared `programs-ui/forms` primitives — every program
...
interface RunResponse {
  runId?: string;
  /** Legacy /api/programs/dragnet/run shape. */
  phraseId?: string;
...
export function DragnetRunForm(): ReactNode {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      // (Phase 3 of the AE review). The legacy /api/programs/dragnet/run
      ...
      router.push(body.receiptUrl ?? `/programs/dragnet/runs/${body.runId}`);
    } catch (err) {
      system.facts.errorMessage =
  ...
  return (
    <form onSubmit={onSubmit} data-testid="dragnet-run-form">
      ...
      <StudioLabel text="Target endpoint">
        <StudioInput
          type="url"
          name="targetUrl"
          required
      ...
      <StudioLabel text="Probe-pack ID">
        <StudioInput
          type="text"
          name="probePackId"
          required
