// ---------------------------------------------------------------------------
// programs-ui/forms — shared form primitives across program flows
// ---------------------------------------------------------------------------
//
// DRAGNET, OATH, TRIPWIRE, and the rest of the 11 alpha programs share
// the same activation-form chrome: target field, secondary identifier
// (probe-pack ID / oath URL / etc.), cadence radio, ToS-ack checkbox,
// disclosure footer, submit button. Each program has DIFFERENT field
// shapes, but the VISUAL primitives are constant.
//
// Per the architecture-review minor in R1+R2 ("inline-style duplication
// across forms"), this file is the single source of truth for those
// primitives. New programs import from here; if a primitive drifts,
// every program's UX drifts together.
// ---------------------------------------------------------------------------

import type {
  ChangeEvent,
  CSSProperties,
  ReactNode,
} from "react";

const InputStyle: CSSProperties = {
  width: "100%",
  padding: "8px 12px",
  fontFamily: "var(--studio-mono)",
  fontSize: 14,
  background: "var(--studio-bg)",
  color: "var(--studio-fg)",
  border: "1px solid var(--studio-fg-dim)",
  borderRadius: 4,
  marginTop: 4,
};

const LabelStyle: CSSProperties = {
  display: "block",
  marginTop: 16,
  fontFamily: "var(--studio-mono)",
  fontSize: 13,
  color: "var(--studio-fg-dim)",
  textTransform: "uppercase",
  letterSpacing: "0.06em",
};

const HelpTextStyle: CSSProperties = {
  fontSize: 12,
  color: "var(--studio-fg-dim)",
  marginTop: 4,
};

const ButtonStyle: CSSProperties = {
  marginTop: 24,
  padding: "10px 20px",
  fontFamily: "var(--studio-mono)",
  fontSize: 14,
  background: "var(--studio-fg)",
  color: "var(--studio-bg)",
  border: "none",
  borderRadius: 4,
  cursor: "pointer",
};

interface StudioInputProps {
  type?: "text" | "url" | "email";
  name: string;
  required?: boolean;
  autoFocus?: boolean;
  placeholder?: string;
  value: string;
  onChange: (v: string) => void;
  testId?: string;
}

export function StudioInput({
  type = "text",
  name,
  required,
  autoFocus,
  placeholder,
  value,
  onChange,
  testId,
}: StudioInputProps): ReactNode {
  return (
    <input
      type={type}
      name={name}
      required={required}
      autoFocus={autoFocus}
      placeholder={placeholder}
      value={value}
      onChange={(e: ChangeEvent<HTMLInputElement>) => onChange(e.target.value)}
      style={InputStyle}
      data-testid={testId}
    />
  );
}

const TextareaStyle: CSSProperties = {
  width: "100%",
  padding: "8px 12px",
  fontFamily: "var(--studio-mono)",
  fontSize: 14,
  background: "var(--studio-bg)",
  color: "var(--studio-fg)",
  border: "1px solid var(--studio-fg-dim)",
  borderRadius: 4,
  marginTop: 4,
  resize: "vertical",
  minHeight: 80,
};

interface StudioTextareaProps {
  name: string;
  required?: boolean;
  placeholder?: string;
  value: string;
  onChange: (v: string) => void;
  rows?: number;
  testId?: string;
}

export function StudioTextarea({
  name,
  required,
  placeholder,
  value,
  onChange,
  rows = 3,
  testId,
}: StudioTextareaProps): ReactNode {
  return (
    <textarea
      name={name}
      required={required}
      placeholder={placeholder}
      value={value}
      onChange={(e) => onChange(e.target.value)}
      rows={rows}
      style={TextareaStyle}
      data-testid={testId}
    />
  );
}

interface StudioLabelProps {
  text: string;
  children: ReactNode;
}

export function StudioLabel({
  text,
  children,
}: StudioLabelProps): ReactNode {
  return (
    <label style={LabelStyle}>
      {text}
      {children}
    </label>
  );
}

interface StudioHelpTextProps {
  children: ReactNode;
  /**
   * When true, the help text takes on the form's error color — used
   * for inline field-level validation feedback (e.g. malformed cron).
   */
  error?: boolean;
  testId?: string;
}

export function StudioHelpText({
  children,
  error = false,
  testId,
}: StudioHelpTextProps): ReactNode {
  const style = error
    ? { ...HelpTextStyle, color: "#ff8888" }
    : HelpTextStyle;

  return (
    <p style={style} data-testid={testId}>
      {children}
    </p>
  );
}

interface StudioCheckboxProps {
  checked: boolean;
  onChange: (v: boolean) => void;
  children: ReactNode;
  testId?: string;
}

export function StudioCheckbox({
  checked,
  onChange,
  children,
  testId,
}: StudioCheckboxProps): ReactNode {
  return (
    <label style={{ display: "block", marginTop: 24 }}>
      <input
        type="checkbox"
        checked={checked}
        onChange={(e) => onChange(e.target.checked)}
        data-testid={testId}
      />{" "}
      {children}
    </label>
  );
}

interface StudioRadioOption<T extends string> {
  value: T;
  label: ReactNode;
  disabled?: boolean;
  testId?: string;
}

interface StudioRadioGroupProps<T extends string> {
  name: string;
  legend: string;
  options: ReadonlyArray<StudioRadioOption<T>>;
  value: T;
  onChange: (v: T) => void;
  testId?: string;
}

export function StudioRadioGroup<T extends string>({
  name,
  legend,
  options,
  value,
  onChange,
  testId,
}: StudioRadioGroupProps<T>): ReactNode {
  return (
    <fieldset
      style={{ marginTop: 16, border: "none", padding: 0 }}
      data-testid={testId}
    >
      <legend style={LabelStyle}>{legend}</legend>
      {options.map((opt) => (
        <label
          key={opt.value}
          style={{
            display: "block",
            marginTop: 8,
            ...(opt.disabled
              ? { color: "var(--studio-fg-dim)" }
              : undefined),
          }}
        >
          <input
            type="radio"
            name={name}
            value={opt.value}
            checked={value === opt.value}
            disabled={opt.disabled}
            onChange={() => onChange(opt.value)}
            data-testid={opt.testId}
          />{" "}
          {opt.label}
        </label>
      ))}
    </fieldset>
  );
}

interface StudioButtonProps {
  type?: "button" | "submit";
  disabled?: boolean;
  children: ReactNode;
  testId?: string;
  onClick?: () => void;
}

export function StudioButton({
  type = "button",
  disabled,
  children,
  testId,
  onClick,
}: StudioButtonProps): ReactNode {
  return (
    <button
      type={type}
      disabled={disabled}
      onClick={onClick}
      style={{ ...ButtonStyle, opacity: disabled ? 0.6 : 1 }}
      data-testid={testId}
    >
      {children}
    </button>
  );
}

interface SignInPromptProps {
  signInUrl: string;
  /**
   * What the user was about to do, in verb-phrase form ("run a
   * probe-pack", "verify this oath"). Defaults to a generic phrase.
   * Caller-provided so the copy reads correctly per program.
   */
  action?: string;
  testId?: string;
}

export function StudioSignInPrompt({
  signInUrl,
  action = "continue",
  testId,
}: SignInPromptProps): ReactNode {
  return (
    <p
      style={{ marginTop: 16, color: "var(--studio-fg-dim)" }}
      data-testid={testId}
    >
      You need to be signed in to {action}.{" "}
      <a href={signInUrl}>Sign in</a> and try again.
    </p>
  );
}

interface StudioErrorProps {
  message: string;
  testId?: string;
}

export function StudioError({ message, testId }: StudioErrorProps): ReactNode {
  return (
    <p
      style={{ marginTop: 16, color: "#ff4444" }}
      data-testid={testId}
      role="alert"
      aria-live="polite"
    >
      {message}
    </p>
  );
}
