...
// shared programs-ui/forms primitives), different field set:
//
...
interface RunResponse {
  runId?: string;
  /** Legacy /api/programs/oath/run shape. */
  phraseId?: string;
...
export function OathRunForm(): ReactNode {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      // /api/programs/oath/run route stays alive as a deprecated alias
      ...
      router.push(body.receiptUrl ?? `/programs/oath/runs/${body.runId}`);
    } catch (err) {
      system.facts.errorMessage =
  ...
  return (
    <form onSubmit={onSubmit} data-testid="oath-run-form">
      <StudioLabel text="Vendor domain">
        <StudioInput
          type="text"
          name="vendorDomain"
          required
      ...
      <StudioLabel text="Hosting origin (optional)">
        <StudioInput
          type="url"
          name="hostingOrigin"
