...
interface RunResponse {
  runId?: string;
  /** Legacy /api/programs/whistle/run shape. */
  phraseId?: string;
...
export function WhistleRunForm(): ReactNode {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      router.push(body.receiptUrl ?? `/programs/whistle/runs/${body.runId}`);
    } catch (err) {
      system.facts.errorMessage =
  ...
  return (
    <form onSubmit={onSubmit} data-testid="whistle-run-form">
      ...
      <StudioLabel text="Manual-redact phrase (optional)">
        <StudioTextarea
      ...
      >
        ...
        <p style={{ fontSize: 13, marginBottom: 0 }}>
          Read{" "}
          <a href="/programs/whistle">
            <code>/programs/whistle</code>
          </a>{" "}
