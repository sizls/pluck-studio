...
export const alt = "BOUNTY filing receipt";
...
const FG_DIM = "#888273";
const BG = "#1a1a1a";
const ACCENT = "#1f7a3a"; // BOUNTY green — landing's bounty-payout callout
...
export default async function Image({ params }: OgProps): Promise<Response> {
  ...
  const target = vendorFromPhrase(id);
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", flex: 1 }}>
          {target !== null ? (
            <div style={{ fontSize: 36, color: FG_DIM, marginBottom: 12, letterSpacing: "0.04em" }}>
              Filed on <strong style={{ color: FG }}>{target}</strong>
            </div>
          ) : null}
          ...
          <div style={{ marginTop: 28, fontSize: 28, color: FG_DIM, letterSpacing: "0.06em" }}>
            filing pending — anchored to Sigstore Rekor on success
          </div>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", color: FG_DIM, fontSize: 20 }}>
          <span>studio.pluck.run</span>
          <span style={{ letterSpacing: "0.08em" }}>EvidencePacket · BountySubmission · auth-token-local</span>
        </div>
      </div>
    ),
    { ...size },
